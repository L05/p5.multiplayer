...
function newConnection(socket) {
	...
	function rerouteRotation(data) {
		...
		if (client != null) {
			io.sockets.in(data.roomId).emit('rotation', {id: socket.id, rotX: data.rotX, rotY: data.rotY, rotZ: data.rotZ});
		}
	}
	...
	function rerouteTrackPad(data) {
		...
		if (client != null) {
			io.sockets.in(data.roomId).emit('trackPad', {id: socket.id, padX: data.padX, padY: data.padY});
			// console.log("id: " + socket.id + ", padX: " + data.padX + ", padY: " + data.padY);
		}
	}
	...
	function rerouteMode(data) {
		...
		if (client != null) {
			io.sockets.in(data.roomId).emit('mode', {id: socket.id, mode: data.mode});
			console.log(socket.id + " in " + data.roomId + ": " + data.mode);
		}
	}
